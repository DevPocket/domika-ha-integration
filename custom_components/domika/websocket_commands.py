...
from .functions import get_critical_sensors, json_encoder_domika, make_dictionary
...
def websocket_domika_update_app_session(
    ...
    pusher = push.Pusher('')
    app_session_id = pusher.update_app_session_id(msg.get('app_session_id'))
    connection.send_result(msg.get('id'), {'app_session_id': app_session_id})
...
def websocket_domika_update_push_token(
    ...
    res = pusher.update_push_notification_token(
        app_session_id, msg.get('push_token_hex'), msg.get('platform'), msg.get('environment')
    )
    pusher.close_connection()
    connection.send_result(msg.get('id'), {'result': res})
...
def websocket_domika_remove_app_session(
    ...
    pusher = push.Pusher('')
    pusher.remove_app_session_id(msg.get('app_session_id'))
    pusher.close_connection()
    connection.send_result(msg.get('id'), remove_push_session(msg.get('app_session_id')))
...
def websocket_domika_verify_push_session(
    ...
    if app_session_id and verification_key:
        ...
        r = requests.post(
            'https://domika.app/verify_push_session',
            json={
                'verification_key': verification_key,
                'old_push_session_id': old_push_session_id,
            },
        )
        ...
        connection.send_result(
            ...
            {
                'result': 0,
                'text': 'one of the parameters is missing',
            },
        )
...
def remove_push_session(app_session_id):
    if app_session_id:
        ...
        if push_session_id:
            r = requests.post(
                'https://domika.app/remove_push_session',
        ...
        return {
            'result': 0,
            'text': f'push_session_id not found for app_session_id: {app_session_id}',
        }

    return {
        'result': 0,
        'text': 'missing app_session_id',
    }
    ...
    if app_session_id:
        for entity_id in msg.get('subscriptions'):
            ...
            if state:
                ...
                LOGGER.error(
                    f'websocket_domika_resubscribe requesting state of unknown entity: {entity_id}',
                )

    # LOGGER.debug(f"### websocket_domika_resubscribe: {app_session_id}, {res_list} ")

    connection.send_result(msg.get('id'), {'entities': res_list})

    pusher = push.Pusher('')
    pusher.resubscribe(app_session_id, msg.get('subscriptions'))
    pusher.close_connection()
...
def websocket_domika_resubscribe_push(
    ...
    pusher = push.Pusher('')
    pusher.resubscribe_push(app_session_id, msg.get('subscriptions'))
    pusher.close_connection()
...
@websocket_command({vol.Required('type'): 'domika/critical_sensors'})
@callback
def websocket_domika_critical_sensors(
    ...
    connection.send_result(msg.get('id'), sensors_data)
    pusher.close_connection()
...
def websocket_domika_get_dashboards(
    ...
    connection.send_result(msg.get('id'), dashboards)
    pusher.close_connection()
